/*
 * Prepared processor-bus response controller.
 *
 * PIO owns the clock, exact early-T1 matching, and scattered AD/PINDIRS.
 * DMA feeds bounded key/response tables from SRAM, so M33 software is absent
 * from every current-cycle response. A passive PIO/DMA observer retains bus
 * evidence independently of Host-visible results.
 *
 * Unexpected cycles remain high-Z and do not advance descriptor streams.
 * Do not add an M33 lookup or speculative current-cycle fallback. Before and
 * after a bounded run, RESET is high, CLK is low, and AD is SIO input/high-Z.
 * See ADR 0003 for the physical timing boundary.
 */
...
#include "bus_phase_capture.pio.h"
#include "prepared_bus_responder.pio.h"
#include "bus_observer.pio.h"

/* CMake supplies the canonical native processor image. */
#ifndef RP86_PROCESSOR_IMAGE_HEADER
#define RP86_PROCESSOR_IMAGE_HEADER "processor_runtime_image.h"
#define RP86_PROCESSOR_IMAGE_DATA processor_runtime_image_data
#define RP86_PROCESSOR_IMAGE_SIZE processor_runtime_image_size
#define RP86_PROCESSOR_IMAGE_SHA256 processor_runtime_image_sha256
#endif
...
#define RP86_PROCESSOR_HZ                 300000u
#endif
...
#ifndef RP86_PREPARED_SEQUENCE_MAX
#define RP86_PREPARED_SEQUENCE_MAX                      32u
#endif
...
#define QUALIFIED_T1_CONTROL_BITS ((1u << RP86_PROCESSOR_PIN_ASTB) | \
...
static uint32_t g_sequence_keys[RP86_PREPARED_SEQUENCE_MAX];
static uint32_t g_sequence_responses[RP86_PREPARED_SEQUENCE_MAX];
...
static lane_mask_t decode_lanes(uint32_t raw) {
    ...
    const bool a0 = sample_bit(raw, RP86_PROCESSOR_PIN_AD0) != 0u;
    const bool ube_n = sample_bit(raw, RP86_PROCESSOR_PIN_UBE) != 0u;
...
static bool is_memory_read(uint32_t raw) {
    ...
    return sample_bit(raw, RP86_PROCESSOR_PIN_IOM) != 0u &&
           sample_bit(raw, RP86_PROCESSOR_PIN_BUFRW) == 0u &&
           sample_bit(raw, RP86_PROCESSOR_PIN_INTAK) != 0u;
}
...
static uint64_t timeout_us_from_clocks(uint32_t clocks) {
    ...
    return ((uint64_t)clocks * 1000000ull + RP86_PROCESSOR_HZ - 1u) /
           RP86_PROCESSOR_HZ + 2u;
}
...
static bool ad_is_sio_high_z(void) {
    if ((sio_hw->gpio_oe & RP86_PROCESSOR_AD_BUS_MASK) != 0u) return false;
    for (uint bit = 0u; bit < 16u; ++bit) {
...
static void route_ad_to_responder(const rp86_pio_sm_t *responder) {
...
static bool responder_non_ad_pins_isolated(const rp86_pio_sm_t *responder) {
...
static void clock_init(rp86_pio_sm_t *clock) {
    clock->pio = pio1;
    clock->sm = pio_claim_unused_sm(clock->pio, true);
    clock->offset = pio_add_program(clock->pio, &rp86_free_running_clock_program);
}

static void clock_prepare(rp86_pio_sm_t *clock) {
    ...
    pio_sm_config c = rp86_free_running_clock_program_get_default_config(clock->offset);
    sm_config_set_set_pins(&c, RP86_PROCESSOR_PIN_CLK, 1u);
    sm_config_set_clkdiv(&c,
        (float)clock_get_hz(clk_sys) / (2.0f * (float)RP86_PROCESSOR_HZ));
    ...
    pio_sm_set_pins_with_mask(clock->pio, clock->sm, 0u, 1u << RP86_PROCESSOR_PIN_CLK);
}

static void clock_start(rp86_pio_sm_t *clock) {
    clock_prepare(clock);
    pio_sm_set_enabled(clock->pio, clock->sm, true);
}

static void clock_stop_low(rp86_pio_sm_t *clock) {
...
static bool wait_reset_clocks(uint count) {
     ...
     * is disabled. This creates the clean RESET-only controller epoch
    ...
    for (uint i = 0u; i < count; ++i) {
        ...
        while (time_us_64() <= deadline && !gpio_get(RP86_PROCESSOR_PIN_CLK))
            tight_loop_contents();
...
static void matcher_init(rp86_pio_sm_t *matcher) {
    ...
    matcher->offset = pio_add_program(matcher->pio,
                                      &rp86_prepared_bus_matcher_program);
    pio_sm_config c =
        rp86_prepared_bus_matcher_program_get_default_config(matcher->offset);
...
static void responder_init(rp86_pio_sm_t *responder) {
    ...
    responder->offset = pio_add_program(
        responder->pio, &rp86_prepared_bus_responder_program);
    pio_sm_config c = rp86_prepared_bus_responder_program_get_default_config(
        responder->offset);
...
static void phase_capture_init(rp86_pio_sm_t *phase) {
    ...
    phase->offset = pio_add_program(
        phase->pio, &rp86_bus_phase_capture_program);
    pio_sm_config c = rp86_bus_phase_capture_program_get_default_config(
        phase->offset);
    sm_config_set_in_pins(&c, 0u);
...
static void observer_init(rp86_pio_sm_t *observer) {
    ...
    observer->offset = pio_add_program(
        observer->pio, &rp86_bus_observer_program);
    pio_sm_config c =
        rp86_bus_observer_program_get_default_config(
            observer->offset);
    sm_config_set_in_pins(&c, 0u);
    sm_config_set_fifo_join(&c, PIO_FIFO_JOIN_RX);
...
static void arm_sm(rp86_pio_sm_t *sm) {
...
static void responder_preserve_clock_direction(rp86_pio_sm_t *responder) {
    ...
    pio_sm_put_blocking(responder->pio, responder->sm,
                        1u << (RP86_PROCESSOR_PIN_CLK - OUT_BASE));
...
static int start_pio_tx_dma(const rp86_pio_sm_t *sm, const uint32_t *table,
                            uint32_t count) {
...
static int start_observer_dma(const rp86_pio_sm_t *observer) {
...
static bool wait_fifo_primed(const rp86_pio_sm_t *sm, uint32_t level) {
...
static bool lookup_rom_word(uint32_t address, uint16_t *word) {
    ...
    } else if (address >= PROCESSOR_IMAGE_BASE &&
               address < PROCESSOR_IMAGE_BASE + RP86_PROCESSOR_IMAGE_SIZE) {
        bytes = RP86_PROCESSOR_IMAGE_DATA;
        size = RP86_PROCESSOR_IMAGE_SIZE;
        offset = address - PROCESSOR_IMAGE_BASE;
    } else {
        return false;
    }
...
static void append_response(uint32_t address, uint16_t word) {
    ...
    hard_assert(g_sequence_count < RP86_PREPARED_SEQUENCE_MAX);
...
static void prepare_response_tables(void) {
    ...
    for (uint32_t offset = 0u; offset < RP86_PROCESSOR_IMAGE_SIZE; offset += 2u) {
        uint16_t word = 0u;
        hard_assert(lookup_rom_word(PROCESSOR_IMAGE_BASE + offset, &word));
        append_response(PROCESSOR_IMAGE_BASE + offset, word);
    }
    hard_assert(RP86_PROCESSOR_IMAGE_SIZE >= 2u);
    g_checkpoint_address = PROCESSOR_IMAGE_BASE +
                           (uint32_t)RP86_PROCESSOR_IMAGE_SIZE - 2u;
...
static void classify_trace(rp86_prepared_bus_result_t *result) {
    ...
    for (uint i = 0u; i < result->trace_count; ++i) {
        ...
        if (entry->memory_read && address == PROCESSOR_IMAGE_BASE)
...
static void run_test(rp86_pio_sm_t *clock, rp86_pio_sm_t *matcher,
                     rp86_pio_sm_t *phase, rp86_pio_sm_t *observer,
                     rp86_pio_sm_t *responder, rp86_prepared_bus_result_t *result) {
    /* Phase 0 - force a known safe baseline before generating RESET clocks. */
    *result = (rp86_prepared_bus_result_t){0};
    ...
    result->clock_direction_armed =
        (result->pre_pio1_padoe & (1u << RP86_PROCESSOR_PIN_CLK)) != 0u &&
        (result->pre_pio1_padoe & RP86_PROCESSOR_AD_BUS_MASK) == 0u;
    ...
    result->pre_release_clean = matcher_primed && responder_primed &&
        ...
        result->clock_direction_armed && !gpio_get(RP86_PROCESSOR_PIN_CLK) &&
    ...
    route_ad_to_sio_high_z();
    gpio_put(RP86_PROCESSOR_PIN_INTR, false);
    result->terminal_safe = gpio_get(RP86_PROCESSOR_PIN_RESET) &&
                            !gpio_get(RP86_PROCESSOR_PIN_CLK) && ad_is_sio_high_z();
}

static bool result_valid(const rp86_prepared_bus_result_t *result) {
...
static bool result_pass(const rp86_prepared_bus_result_t *result) {
    const bool payload_ok = result->signature_1234 &&
                            result->signature_5678 &&
...
static void print_readable_summary(const rp86_prepared_bus_result_t *result) {
...
static void print_result(const rp86_prepared_bus_result_t *result) {
    /* USB reporting is strictly post-run. None of these formatting paths can
     * perturb a processor-bus deadline or mask the raw physical evidence. */
    print_readable_summary(result);
    printf("\n[ENGINEERING DETAILS]\n");
    printf("\nRP86 Prepared Processor-Bus Execution\n");
    ...
    printf("Bus controller      : PIO1 synchronized CLK + exact matcher + AD/PINDIRS\n");
    ...
    printf("Processor image     : %lu bytes at F0000; SHA-256 %s\n",
           (unsigned long)RP86_PROCESSOR_IMAGE_SIZE, RP86_PROCESSOR_IMAGE_SHA256);
    ...
    for (uint i = 0u; i < result->phase_count; ++i) {
        ...
        printf("%-4s  %08lX   %u   %u   %u   %u    %u   %u  %04X\n",
               ...
               (unsigned)sample_bit(raw, RP86_PROCESSOR_PIN_ASTB),
               (unsigned)sample_bit(raw, RP86_PROCESSOR_PIN_CLK),
               (unsigned)sample_bit(raw, RP86_PROCESSOR_PIN_IOM),
               (unsigned)sample_bit(raw, RP86_PROCESSOR_PIN_BUFRW),
               (unsigned)sample_bit(raw, RP86_PROCESSOR_PIN_INTAK),
               (unsigned)sample_bit(raw, RP86_PROCESSOR_PIN_UBE), decode_ad(raw));
    }

    printf("\nMEASUREMENT EPOCH   = %s\n",
           result_valid(result) ? "VALID" : "INVALID");
    printf("PREPARED BUS RESULT = %s\n",
...
int main(void) {
    ...
    rp86_pio_sm_t clock, matcher, phase, observer, responder;
    clock_init(&clock);
    ...
    static rp86_prepared_bus_result_t result;
