...
static void update_periodic_tick(rp86_workload_executor_t *executor) {
    ...
    if (now >= executor->tick_next_us) {
         ...
         * delivery slot. Periods that expire before its EOI remain observable
         * as generated/delayed/coalesced time, but they must not arm an
         * immediate follow-on request. Otherwise a CLOCK_STEPPED ISR whose
         * wall-clock service time exceeds 10 ms can chain indefinitely and
         * prevent the selected task from executing. */
