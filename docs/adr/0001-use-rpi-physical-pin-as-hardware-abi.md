...
Firmware must centralize processor-bus pin definitions in `firmware/bus/processor_bus_pins.h`. Documentation must identify both the physical header position and the target RP2350 GPIO. Undocumented raw GPIO literals in processor-bus code are prohibited.
...
6. Update `firmware/bus/processor_bus_pins.h` and any absolute PIO pin references.
