...
This makes `py -m host.apps.cli.rp86 --physical-regression C16SMOKE.P86W` suitable as the physical acceptance path: successful lifecycle completion alone is not enough; the firmware-owned formal PASS flag must also have been produced by the native checksum comparison.
