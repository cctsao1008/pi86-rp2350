...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("address", type=_parse_address,
                        help="physical address of gRp86Telemetry, e.g. 0x128D8")
    ...
    try:
        for _attempt in range(5):
            before = decode_sequence(
                read_memory(args.address + TELEMETRY_SEQUENCE_OFFSET, 2)
            )
            if not stable_sequence(before):
                continue
            raw = read_memory(args.address, TELEMETRY_SIZE)
            ...
            if before == after == snapshot.event_sequence and snapshot.stable:
                print(snapshot.format(args.address))
                return 0
