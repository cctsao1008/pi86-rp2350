...
<style>
:root{color-scheme:dark;--bg:#0b0f14;--panel:#111820;--panel2:#0d141b;--line:#26323e;--text:#e7edf3;--muted:#91a0ad;--good:#6fdc8c;--bad:#ff7b72;--warn:#f2cc60;--accent:#7aa2f7}
...
header{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:18px 22px;border-bottom:1px solid var(--line);background:#0d1218;position:sticky;top:0;z-index:10}h1{font-size:18px;margin:0}.sub{color:var(--muted);font-size:12px}
...
.metrics{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px 18px}.metric{display:flex;justify-content:space-between;border-bottom:1px dotted #27323d;padding:5px 0}.metric .k{color:var(--muted)}.metric .v{text-align:right}.hero{font-size:20px;font-weight:700;margin-bottom:12px}.hero small{font-size:12px;font-weight:400;color:var(--muted)}
pre{margin:0;white-space:pre-wrap;word-break:break-word;background:#080c10;border:1px solid #1d2730;border-radius:7px;padding:11px;min-height:115px;max-height:420px;overflow:auto}table{width:100%;border-collapse:collapse}th,td{text-align:left;padding:7px 8px;border-bottom:1px solid var(--line)}th{color:var(--muted);font-weight:600}.muted{color:var(--muted)}.good{color:var(--good)}.bad{color:var(--bad)}.warn{color:var(--warn)}
.placeholder{border:1px dashed #314050;border-radius:8px;padding:16px;color:var(--muted);min-height:115px}.console-line{display:flex;gap:8px;margin-top:9px}.console-line input{flex:1}.small{font-size:12px}@media(max-width:1000px){.card,.card.third{grid-column:span 12}.metrics{grid-template-columns:1fr}header{align-items:flex-start;flex-direction:column}}
</style>
</head>
<body>
<header><div><h1>RP86 Development Console</h1><div class="sub">Intel 8086 / NEC V30 × RP2350 · physical-processor first</div></div><div class="toolbar"><button id="refreshAll">Refresh</button><button id="autoBtn">Auto refresh: ON</button></div></header>
<main>
...
function refreshAll(){refreshProcessor();refreshStatus();scanDevices()}$('refreshAll').onclick=refreshAll;$('devicesBtn').onclick=scanDevices;$('simulateBtn').onclick=simulate;$('rebootBtn').onclick=()=>control('reboot','Reboot RP2350');$('bootBtn').onclick=()=>control('bootloader','Enter UF2 bootloader');$('clearLog').onclick=()=>$('logOut').textContent='';$('autoBtn').onclick=()=>{autoRefresh=!autoRefresh;$('autoBtn').textContent=`Auto refresh: ${autoRefresh?'ON':'OFF'}`};setInterval(()=>{if(autoRefresh){refreshProcessor();refreshStatus()}},2000);refreshAll();
</script>
</body>
</html>
"""
