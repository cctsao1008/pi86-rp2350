...
.runtime-only.card{min-height:0}.runtime-only .card-head{padding:9px 12px}.runtime-only .card-body{padding:10px 12px}.runtime-only .metrics{gap:3px 16px}.runtime-only .metric{padding:3px 0}.runtime-only td{padding:4px 7px}.runtime-summary{grid-column:span 12}.runtime-summary .metrics{grid-template-columns:repeat(3,minmax(0,1fr))}.runtime-only pre{min-height:86px}.runtime-only .statusline{margin-bottom:6px}.control-row{align-items:center}.control-note{margin-left:auto}
@media(max-width:1000px){.card,.card.third,.runtime-summary{grid-column:span 12}.metrics,.runtime-summary .metrics{grid-template-columns:1fr}header{align-items:flex-start;flex-direction:column}.statebar{top:126px}.memory-line{grid-template-columns:1fr}.control-note{margin-left:0;width:100%}}
</style>
</head>
<body>
<header>
...
<section class="card runtime-only"><div class="card-head"><span>Capabilities</span><span class="muted">CURRENT PLATFORM</span></div><div class="card-body"><table><tbody><tr><td>Processor console</td><td class="good cap-state">AVAILABLE</td></tr><tr><td>Memory</td><td class="good cap-state">READ-ONLY</td></tr><tr><td>Bus trace</td><td class="warn cap-state">FIRMWARE-ONLY</td></tr><tr><td>Flash filesystem</td><td class="good cap-state">AVAILABLE</td></tr><tr><td>PSRAM</td><td class="muted cap-state">NOT CONFIGURED</td></tr><tr><td>SD</td><td class="muted cap-state">HARDWARE ONLY</td></tr><tr><td>DVI</td><td class="muted cap-state">NOT IMPLEMENTED</td></tr><tr><td>PIO-USB</td><td class="muted cap-state">NOT IMPLEMENTED</td></tr></tbody></table><div class="muted small" style="margin-top:8px">Bus trace is retained by firmware; structured Host access is not exposed in the current ABI. No CDC scraping is used.</div></div></section>

<section class="card runtime-only"><div class="card-head"><span>Runtime control</span><span class="warn">PHYSICAL HARDWARE</span></div><div class="card-body"><div class="control-row"><button id="rebootBtn">Reboot RP2350</button><button id="bootBtn" class="danger">Enter UF2 bootloader</button><span class="muted small control-note">UF2 stops the current RP86 runtime.</span></div></div></section>
...
function renderProcessor(data){lastProcessorAt=Date.now();const s=data.snapshot||{},native=s.native_processor||null,identified=!!native,names={'nec-v30':'NEC V30','intel-8086':'Intel 8086'},cpuLabel=native?(names[String(native).toLowerCase()]||String(native).toUpperCase()):'8086-class processor',execution=val(s.workload_processor_state,'UNKNOWN'),workload=val(s.workload_state,'EMPTY');$('cpuName').textContent=cpuLabel;$('barCpuName').textContent=native?cpuLabel:'PROCESSOR';$('processorIdentity').textContent=identified?'NATIVE AAD 16 IDENTIFIED':'NOT PROBED';$('barWorkload').textContent=workload;$('workloadState').textContent=workload;$('clockMode').textContent=val(s.workload_clock_mode);$('cpuCycles').textContent=val(s.workload_cycles);const activeCycles=Number(s.workload_cycles||0)>0&&!['EMPTY','STOPPED'].includes(String(workload).toUpperCase());$('cpuCyclesMetric').style.display=activeCycles?'flex':'none';const clockText=String(s.workload_clock_mode||'--').replace('FREE-RUNNING','1.000 MHz');$('overviewClock').textContent=clockText;$('barClock').textContent=clockText;$('runtimeClock').textContent=val(s.workload_clock_mode);$('runtimeWorkload').textContent=workload;$('runtimeState').textContent=val(s.state);$('hostState').textContent='CONNECTED';setBarDot('hostDot','good');const mode=data.owner_mode==='web-owned'?'WEB OWNER':data.owner_mode==='existing'?'ATTACHED':'ACTIVE';$('runtimeSession').textContent=mode;$('runtimeDevice').textContent=val(data.device_id);$('runtimeConnection').textContent='ACTIVE';$('hostSessionBadge').textContent=mode;$('hostSessionBadge').className='good';$('cpuAlive').textContent=`Processor state: ${execution}`;$('cpuBadge').textContent=identified?'IDENTIFIED':execution;$('cpuBadge').className=identified?'good':'warn';$('barCpuState').textContent=execution;setBarDot('barCpuDot',execution==='STOPPED / RESET'?'warn':'good');$('cpuDot').className='dot '+(execution==='STOPPED / RESET'?'warn':'good')}
