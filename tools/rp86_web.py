...
<section class="card overview-only"><div class="card-head"><span>Memory viewer</span><span class="good">READ-ONLY</span></div><div class="card-body"><div class="memory-line"><input id="memoryAddress" value="0x00000" placeholder="Address, e.g. 0x7FF0"><input id="memoryLength" value="32" inputmode="numeric" placeholder="1-40"><button id="memoryRead">Read</button></div><div class="quick" style="margin-bottom:10px"><button data-mem-step="-1">Previous</button><button data-mem-len="16">16</button><button data-mem-len="32">32</button><button data-mem-len="40">40</button><button data-mem-step="1">Next</button></div><pre id="memoryOut">Internal SRAM 00000-3FFFF. Read 1-40 bytes through the existing memory service.</pre></div></section>

<section class="card overview-only"><div class="card-head"><span>Capabilities</span><span class="muted">CURRENT MACHINE</span></div><div class="card-body"><table><tbody><tr><td>Processor console</td><td class="good cap-state">AVAILABLE</td></tr><tr><td>Memory</td><td class="good cap-state">READ-ONLY</td></tr><tr><td>Bus trace</td><td class="warn cap-state">FIRMWARE-ONLY</td></tr><tr><td>Flash filesystem</td><td class="good cap-state">AVAILABLE</td></tr><tr><td>PSRAM</td><td class="muted cap-state">NOT CONFIGURED</td></tr><tr><td>SD</td><td class="muted cap-state">HARDWARE ONLY</td></tr><tr><td>DVI</td><td class="muted cap-state">NOT IMPLEMENTED</td></tr><tr><td>PIO-USB</td><td class="muted cap-state">NOT IMPLEMENTED</td></tr></tbody></table><div class="muted small" style="margin-top:10px">Bus trace is retained by firmware; structured Host access is not exposed in the current ABI. No CDC scraping is used.</div></div></section>

<section class="card overview-only"><div class="card-head"><span>Runtime control</span><span class="warn">PHYSICAL HARDWARE</span></div><div class="card-body"><button id="rebootBtn">Reboot RP2350</button><div class="danger-zone"><div class="bad small" style="margin-bottom:8px">Danger zone</div><button id="bootBtn" class="danger">Enter UF2 bootloader</button><div class="muted small" style="margin-top:8px">UF2 bootloader stops the current RP86 runtime.</div></div></div></section>

<section class="card runtime-only"><div class="card-head"><span>RP2350 runtime</span><span id="runtimeBadge" class="muted">unknown</span></div><div class="card-body"><div class="statusline"><span id="runtimeDot" class="dot"></span><span id="runtimeText">Not checked</span></div><div class="metrics"><div class="metric"><span class="k">Processor</span><span class="v" id="cpuIdentity">UNKNOWN</span></div><div class="metric"><span class="k">Identity policy</span><span class="v" id="identityPolicy">--</span></div><div class="metric"><span class="k">Broker state</span><span class="v" id="cpuState">--</span></div><div class="metric"><span class="k">RTT</span><span class="v" id="rtt">--</span></div><div class="metric"><span class="k">Request sequence</span><span class="v" id="requestSeq">--</span></div><div class="metric"><span class="k">CPU sequence</span><span class="v" id="cpuSeq">--</span></div><div class="metric"><span class="k">Boot ID</span><span class="v" id="bootId">--</span></div><div class="metric"><span class="k">Command sequence</span><span class="v" id="commandSeq">--</span></div><div class="metric"><span class="k">Device</span><span class="v" id="deviceId">--</span></div></div><details><summary>Full RP2350 runtime diagnostic</summary><pre id="statusOut">Press Refresh to query the canonical RP86 runtime.</pre></details></div></section>

<section class="card runtime-only"><div class="card-head"><span>Host broker</span><span id="brokerBadge" class="muted">unknown</span></div><div class="card-body"><div class="metrics"><div class="metric"><span class="k">Session</span><span class="v" id="runtimeSession">--</span></div><div class="metric"><span class="k">Device</span><span class="v" id="runtimeDevice">--</span></div><div class="metric"><span class="k">TCP</span><span class="v" id="runtimeTcp">--</span></div><div class="metric"><span class="k">UDP</span><span class="v" id="runtimeUdp">--</span></div></div><details><summary>Raw broker JSON</summary><pre id="brokerOut">No broker query yet.</pre></details></div></section>

<section class="card runtime-only"><div class="card-head"><span>USB / HID devices</span><button id="devicesBtn">Scan</button></div><div class="card-body"><div id="deviceTable" class="muted">Not scanned</div></div></section>
<section class="card runtime-only"><div class="card-head"><span>Protocol self-test</span><button id="simulateBtn">Run simulation</button></div><div class="card-body"><pre id="simulateOut">Not run</pre></div></section>
...
<script>
const $=id=>document.getElementById(id);let autoRefresh=true,busyStatus=false,busyProcessor=false,lastProcessorAt=0,lastStatusAt=0,consoleHistory=[],consoleHistoryIndex=0;
...
function setCpuOffline(reason){$('cpuDot').className='dot bad';$('cpuAlive').textContent=reason||'No live broker telemetry';$('cpuBadge').textContent='NO BROKER';$('cpuBadge').className='warn';$('brokerMode').textContent='runtime-only view';$('cpuIdentity').textContent='UNKNOWN';$('hostState').textContent='DISCONNECTED';setBarDot('hostDot','bad');$('barCpuState').textContent='UNKNOWN';setBarDot('barCpuDot','');$('sessionMode').textContent='--'}
function renderProcessor(data){lastProcessorAt=Date.now();const s=data.snapshot||{},native=s.native_processor||null,policy=(data.processor||'auto').toLowerCase(),completed=Number(s.completed||0),liveness=!!native&&completed>0;const names={'nec-v30':'NEC V30','intel-8086':'Intel 8086'},cpuLabel=native?(names[String(native).toLowerCase()]||String(native).toUpperCase()):'8086-class processor';$('cpuName').textContent=cpuLabel;$('cpuIdentity').textContent=native?cpuLabel:'UNKNOWN';$('barCpuName').textContent=native?cpuLabel:'PROCESSOR';$('identityPolicy').textContent=policy==='auto'?'AUTO DETECT':policy.toUpperCase();$('cpuState').textContent=val(s.state);$('heartbeat').textContent=`${completed} completed / ${val(s.lost,0)} lost`;$('barHeartbeat').textContent=`${completed}/${val(s.lost,0)}`;$('rtt').textContent=completed?`${Number(s.last_ms||0).toFixed(1)} ms`:'--';$('requestSeq').textContent=val(s.request_sequence);$('cpuSeq').textContent=val(s.cpu_sequence);$('deviceId').textContent=val(data.device_id);$('bootId').textContent=val(s.boot_id);$('commandSeq').textContent=val(s.command_sequence);$('workloadId').textContent=val(s.workload_id);$('workloadState').textContent=val(s.workload_state);$('clockMode').textContent=val(s.workload_clock_mode);$('cpuCycles').textContent=val(s.workload_cycles);$('overviewClock').textContent=val(s.workload_clock_mode);$('barClock').textContent=String(s.workload_clock_mode||'--').replace('FREE-RUNNING','1.000 MHz');$('hostState').textContent='CONNECTED';setBarDot('hostDot','good');const mode=data.owner_mode==='web-owned'?'WEB OWNER':data.owner_mode==='existing'?'ATTACHED':'ACTIVE';$('sessionMode').textContent=mode;$('runtimeSession').textContent=mode;$('runtimeDevice').textContent=val(data.device_id);$('runtimeTcp').textContent=val(data.tcp_port);$('runtimeUdp').textContent=val(data.udp_port);$('brokerMode').textContent=`${data.owner_mode==='web-owned'?'Web-owned background runtime':data.owner_mode==='existing'?'Existing Host Broker':'Host Broker'} · ${val(data.device_id)}`;$('brokerBadge').textContent=mode;$('brokerBadge').className='good';$('brokerOut').textContent=JSON.stringify(data,null,2);if(liveness){$('cpuDot').className='dot good';$('cpuAlive').textContent='Physical processor liveness proven';$('cpuBadge').textContent='ALIVE';$('cpuBadge').className='good';$('barCpuState').textContent='ALIVE';setBarDot('barCpuDot','good')}else{$('cpuDot').className='dot warn';$('cpuAlive').textContent='Host Broker active; CPU liveness not yet proven';$('cpuBadge').textContent='UNPROVEN';$('cpuBadge').className='warn';$('barCpuState').textContent='UNPROVEN';setBarDot('barCpuDot','warn')}updateAges()}
