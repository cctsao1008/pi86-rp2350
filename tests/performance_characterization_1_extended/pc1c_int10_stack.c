/*
 * PC1-C0C1-B2-A two-epoch bounded INT 10h and stack proof.
 *
 ...
 *
 * Epoch A learns the real FLAGS/CS/IP words written by INT 10h while stack
 * reads remain unsupported. Under RESET, firmware adds those three captured
 * words to a fresh immutable table. Epoch B must use them to complete IRET and
 * reach the ROM checkpoint. The M33 never services a current bus cycle.
 */
...
#define IVT10_OFFSET_ADDRESS            0x00040u
#define IVT10_SEGMENT_ADDRESS           0x00042u
#define INT10_HANDLER_OFFSET            0x0018u
#define STACK_IP_ADDRESS                0x007FAu
#define STACK_CS_ADDRESS                0x007FCu
#define STACK_FLAGS_ADDRESS             0x007FEu
#define CHECKPOINT_ADDRESS              0xF000Cu
...
static bool g_replay_stack;
static uint16_t g_stack_ip;
static uint16_t g_stack_cs;
static uint16_t g_stack_flags;
...
static bool rom_word(uint32_t address, uint16_t *value) {
    ...
    if (address >= BIOS_BASE && address + 1u < BIOS_BASE + int10_stack_rom_size) {
        const uint32_t i = address - BIOS_BASE;
        *value = (uint16_t)int10_stack_rom_data[i] |
                 ((uint16_t)int10_stack_rom_data[i + 1u] << 8);
        return true;
    }
    if (address == IVT10_OFFSET_ADDRESS) { *value = INT10_HANDLER_OFFSET; return true; }
    if (address == IVT10_SEGMENT_ADDRESS) { *value = 0xF000u; return true; }
    if (g_replay_stack && address == STACK_IP_ADDRESS) { *value = g_stack_ip; return true; }
    if (g_replay_stack && address == STACK_CS_ADDRESS) { *value = g_stack_cs; return true; }
    if (g_replay_stack && address == STACK_FLAGS_ADDRESS) { *value = g_stack_flags; return true; }
    return false;
}
...
static void prepare_table_stream(bool replay_stack) {
    g_replay_stack = replay_stack;
    const uint32_t image_words = (int10_stack_rom_size + 1u) / 2u;
    hard_assert(int10_stack_rom_size == 30u);
    const uint32_t live_words = 3u + image_words + 2u + (replay_stack ? 3u : 0u);
    hard_assert(live_words <= 23u);

    uint32_t prototype[BLOCK_WORDS];
    uint32_t slot = 0u;
    const uint32_t dummy_count = TABLE_ENTRIES - live_words;
    ...
    append_entry(prototype, slot++, IVT10_OFFSET_ADDRESS, INT10_HANDLER_OFFSET);
    append_entry(prototype, slot++, IVT10_SEGMENT_ADDRESS, 0xF000u);
    if (replay_stack) {
        append_entry(prototype, slot++, STACK_IP_ADDRESS, g_stack_ip);
        append_entry(prototype, slot++, STACK_CS_ADDRESS, g_stack_cs);
        append_entry(prototype, slot++, STACK_FLAGS_ADDRESS, g_stack_flags);
    }
...
static void classify(result_t *r) {
    ...
    for (uint32_t i = 0u; i < r->complete_cycles; ++i) {
        ...
        if (address == CHECKPOINT_ADDRESS && memory_read(a)) ++r->checkpoint_reads;
        if (address == DIAGNOSTIC_PORT && io_write(a) && diag + 1u < sizeof r->diagnostic)
            r->diagnostic[diag++] = (char)(decode_ad(d) >> 8);
        if (memory_write(a)) {
            if (address == STACK_IP_ADDRESS && !(r->stack_write_mask & 1u)) {
                r->stack_ip = decode_ad(d); r->stack_write_mask |= 1u;
            } else if (address == STACK_CS_ADDRESS && !(r->stack_write_mask & 2u)) {
                r->stack_cs = decode_ad(d); r->stack_write_mask |= 2u;
            } else if (address == STACK_FLAGS_ADDRESS && !(r->stack_write_mask & 4u)) {
                r->stack_flags = decode_ad(d); r->stack_write_mask |= 4u;
            }
        }
    }
    r->diagnostic[diag] = '\0';
    r->diagnostic_ok = diag >= 1u && r->diagnostic[0] == 'I';
    r->checkpoint_ok = r->checkpoint_reads >= 4u;
    r->stack_writes_ok = r->stack_write_mask == 7u;
    if (g_replay_stack)
        r->stack_writes_ok = r->stack_writes_ok &&
            r->stack_ip == g_stack_ip && r->stack_cs == g_stack_cs &&
            r->stack_flags == g_stack_flags;
}
...
static void print_result(const result_t *r, const char *epoch) {
    bool pass = r->reset_ok && r->clean_epoch && r->first_address_ok &&
        r->first_response_ok && r->far_target_seen && r->diagnostic_ok &&
        r->stack_writes_ok && (!g_replay_stack || r->checkpoint_ok) &&
        r->supported_reads_ok && r->terminal_safe &&
    ...
    printf("Stack writes IP/CS/FLAGS %s (%04X/%04X/%04X)\n",
           pf(r->stack_writes_ok), r->stack_ip, r->stack_cs, r->stack_flags);
    printf("IRET checkpoint          %s (%lu reads)\n",
           g_replay_stack ? pf(r->checkpoint_ok) : "LEARN ONLY",
           (unsigned long)r->checkpoint_reads);
    printf("Bus ownership/safety     %s\n", pf(r->terminal_safe));
    printf("C0C1-B2-A %s RESULT      %s\n", epoch, pf(pass));
    printf("\n[ENGINEERING DETAILS]\n");
    printf("PC1-C0C1-B2-A Bounded INT 10h Stack - 0.300 MHz\n");
    printf("Epoch                    = %s\n", epoch);
    ...
    printf("ROM image                = %lu bytes; SHA-256 %s\n",
           (unsigned long)int10_stack_rom_size, int10_stack_rom_sha256);
...
int main(void) {
    header_high_z(); controls_init(); ad_to_sio();
    ...
    prepare_table_stream(false);
    run(&clock, &response, &reset_qualifier, &observer, &phase, &learn);
    print_result(&learn, "EPOCH-A LEARN"); fflush(stdout);

    if (learn.stack_writes_ok) {
        g_stack_ip = learn.stack_ip;
        g_stack_cs = learn.stack_cs;
        g_stack_flags = learn.stack_flags;
        prepare_table_stream(true);
        run(&clock, &response, &reset_qualifier, &observer, &phase, &replay);
        print_result(&replay, "EPOCH-B REPLAY");
        printf("\nC0C1-B2-A OVERALL RESULT = %s\n",
               pf(replay.reset_ok && replay.clean_epoch &&
                  replay.first_address_ok && replay.first_response_ok &&
                  replay.far_target_seen && replay.diagnostic_ok &&
                  replay.stack_writes_ok && replay.checkpoint_ok &&
                  replay.supported_reads_ok && replay.terminal_safe &&
                  replay.observer_words == OBSERVER_WORDS));
    } else {
        printf("\nEPOCH-B REPLAY = NOT RUN (stack-write learning failed)\n");
        printf("C0C1-B2-A OVERALL RESULT = FAIL\n");
    }
    fflush(stdout);
