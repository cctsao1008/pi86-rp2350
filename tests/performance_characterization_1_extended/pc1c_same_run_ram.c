/*
 * PC1-C0C1-B2-B same-run coherent one-slot RAM proof.
 *
 ...
 *
 * Epoch B has no M33 work in a current bus cycle. PIO1 captures the V30 write
 * to physical 00100h directly at R2, retains the scattered GPIO word in ISR,
 * and replays it on the later read. The V30 mirrors the value to 00102h for
 * passive verification. This is one dynamic RAM slot, not general RAM.
 */
...
typedef struct {
    bool reset_ok, clean_epoch, tx_primed, pio2_clock_only;
    bool clock_low_before_release, first_address_ok, first_response_ok;
    bool far_target_seen, supported_reads_ok, ram_write_ok, ram_read_ok;
    bool mirror_write_ok, checkpoint_ok, terminal_safe;
    ...
    uint16_t ram_write_value, ram_read_value, mirror_write_value;
} result_t;
...
static bool rom_word(uint32_t address, uint16_t *value) {
    ...
    if (address >= BIOS_BASE &&
        address + 1u < BIOS_BASE + same_run_ram_test_rom_size) {
        uint32_t i = address - BIOS_BASE;
        *value = (uint16_t)same_run_ram_test_rom_data[i] |
                 ((uint16_t)same_run_ram_test_rom_data[i + 1u] << 8);
        return true;
    }
    return false;
}
...
static void prepare_bounded_table(void) {
    const uint32_t image_words = same_run_ram_test_rom_size / 2u;
    hard_assert((same_run_ram_test_rom_size & 1u) == 0u);
...
/* Keep only supported ROM reads and the one RAM write/read. Unsupported
 * cycles never consume a key and therefore cannot authorize PINDIRS. */
static bool compile_dynamic_sequence(uint32_t observer_words) {
    g_sequence_count = 0u;
    bool write_added = false, read_added = false;
    ...
    for (uint32_t i = 0u; i < cycles && g_sequence_count < MAX_SEQUENCE; ++i) {
        ...
        if (memory_read(raw) && rom_word(address, &value)) {
            ...
            include = true;
        } else if (!write_added && memory_write(raw) && address == RAM_SLOT_ADDRESS) {
            descriptor = DESC_DYNAMIC;
            write_added = true; include = true;
        } else if (!read_added && memory_read(raw) && address == RAM_SLOT_ADDRESS) {
            descriptor = DESC_DYNAMIC | DESC_DRIVE;
            read_added = true; include = true;
        }
        if (include) {
    ...
    return write_added && read_added && g_sequence_count >= 8u &&
...
static void classify(result_t *r) {
    ...
    for (uint32_t i = 0u; i < r->complete_cycles; ++i) {
        ...
        if (memory_read(a) && address == CHECKPOINT_WORD_ADDRESS)
            ++r->checkpoint_reads;
        if (memory_write(a) && address == RAM_SLOT_ADDRESS) {
            r->ram_write_value = decode_ad(d);
            r->ram_write_ok = r->ram_write_value == RAM_TEST_WORD;
        }
        if (memory_read(a) && address == RAM_SLOT_ADDRESS) {
            r->ram_read_value = decode_ad(d);
            r->ram_read_ok = r->ram_read_value == RAM_TEST_WORD;
        }
        if (memory_write(a) && address == RAM_MIRROR_ADDRESS) {
            r->mirror_write_value = decode_ad(d);
            r->mirror_write_ok = r->mirror_write_value == RAM_TEST_WORD;
        }
    }
...
static void dynamic_response_init(engine_sm_t *matcher, engine_sm_t *responder) {
    ...
    matcher->offset = pio_add_program(pio1, &pc1c_same_run_ram_matcher_program);
    responder->pio = pio1; responder->sm = pio_claim_unused_sm(pio1, true);
    responder->offset = pio_add_program(pio1, &pc1c_same_run_ram_responder_program);
    hard_assert(pc1c_same_run_ram_matcher_program.length +
                pc1c_same_run_ram_responder_program.length == 32u);
    pio_sm_config m = pc1c_same_run_ram_matcher_program_get_default_config(matcher->offset);
    sm_config_set_in_pins(&m, 0u);
    hard_assert(pio_sm_init(pio1, matcher->sm, matcher->offset, &m) == PICO_OK);
    pio_sm_config d = pc1c_same_run_ram_responder_program_get_default_config(responder->offset);
...
static void print_result(const result_t *r, const char *epoch, bool dynamic) {
    bool pass = r->reset_ok && r->clean_epoch && r->first_address_ok &&
        ...
        r->ram_write_ok && r->checkpoint_ok && r->terminal_safe &&
        r->observer_words == OBSERVER_WORDS;
    if (dynamic) pass = pass && r->ram_read_ok && r->mirror_write_ok &&
    ...
    printf("Write 00100=1234         %s (observed %04X)\n",
           pf(r->ram_write_ok), r->ram_write_value);
    printf("Read  00100=1234         %s (observed %04X)\n",
           dynamic ? pf(r->ram_read_ok) : "LEARN ONLY", r->ram_read_value);
    printf("Write 00102=1234         %s (observed %04X)\n",
           dynamic ? pf(r->mirror_write_ok) : "LEARN ONLY", r->mirror_write_value);
    printf("Checkpoint loop          %s (%lu reads)\n", pf(r->checkpoint_ok),
           (unsigned long)r->checkpoint_reads);
    printf("Bus ownership/safety     %s\n", pf(r->terminal_safe));
    printf("C0C1-B2-B %s RESULT      %s\n", epoch, pf(pass));
    printf("\n[ENGINEERING DETAILS]\n");
    printf("PC1-C0C1-B2-B Same-Run One-Slot RAM - 0.300 MHz\n");
    printf("Epoch                    = %s\n", epoch);
    printf("Current-cycle M33        = NONE\n");
    printf("PIO1 program             = %s\n",
           dynamic ? "exact matcher + capture/replay responder (9+23 words)" :
    ...
    printf("ROM image                = %lu bytes; SHA-256 %s\n",
           (unsigned long)same_run_ram_test_rom_size, same_run_ram_test_rom_sha256);
    ...
    uint32_t shown = r->complete_cycles < 32u ? r->complete_cycles : 32u;
    for (uint32_t i = 0u; i < shown; ++i) {
        ...
        printf("%02lu addr=%05lX type=%s data=%04X addr_raw=%08lX data_raw=%08lX\n",
               ...
               memory_read(a) ? "MEMR" : memory_write(a) ? "MEMW" : "OTHER",
...
int main(void) {
    ...
    if (sequence_ok && learn.reset_ok && learn.clean_epoch &&
        ...
        learn.ram_write_ok && learn.checkpoint_ok && learn.terminal_safe) {
        ...
        bool overall = replay.reset_ok && replay.clean_epoch &&
            ...
            replay.ram_write_ok && replay.ram_read_ok && replay.mirror_write_ok &&
        ...
        printf("\nC0C1-B2-B OVERALL RESULT = %s\n", pf(overall));
    } else {
        printf("\nEPOCH-B SAME-RUN = NOT RUN (learning/compile gate failed)\n");
        printf("C0C1-B2-B OVERALL RESULT = FAIL\n");
    }
    fflush(stdout);
